    ...
    <div className="w-full max-w-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white rounded-3xl p-8 border-8 border-poke-red shadow-[0_12px_0_0_rgba(238,21,21,1)] text-poke-dark">
        <div className="flex items-center gap-4 mb-8 border-b-4 border-gray-100 pb-4">
          ...
          <div><h2 className="text-3xl font-black uppercase tracking-tighter">실시간 스피드 계산기</h2><p className="text-gray-500 font-bold italic">Visual Analytics Active!</p></div>
        </div>
        ...
        <div className="mb-8 relative">
          ...
          <div className="relative">
            ...
            <input type="text" placeholder="검색..." value={searchTerm} onChange={(e) => {setSearchTerm(e.target.value); setShowSuggestions(true);}} className="w-full bg-gray-100 border-4 border-transparent focus:border-poke-red outline-none p-3 pl-12 rounded-xl font-bold transition-all" />
            ...
            <div className="absolute z-20 w-full mt-2 bg-white border-4 border-poke-red rounded-xl shadow-xl overflow-hidden animate-in slide-in-from-top-2">
              {suggestions.map((p) => (
                <button key={p.name} onClick={() => selectPokemon(p)} className="w-full p-4 text-left hover:bg-gray-50 font-bold capitalize border-b border-gray-100 flex justify-between">
                  <span>{p.koName || p.name}</span><span className="text-gray-400 text-xs uppercase">{p.name}</span>
                </button>
              ))}
            </div>
          )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            {sprite && (
              <div className="bg-gray-50 rounded-2xl p-4 border-2 border-gray-100 animate-in zoom-in duration-300">
                <div className="flex justify-center mb-4"><img src={sprite} alt="Pokemon" className="w-32 h-32 object-contain" /></div>
                {fullStats && <StatBars stats={fullStats} />}
              </div>
            )}
            <div><label className="block text-xs font-black uppercase mb-1 italic">스피드 종족값 (Base)</label><input type="number" value={baseSpeed} onChange={(e) => setBaseSpeed(Number(e.target.value))} className="w-full bg-gray-100 p-3 rounded-xl font-bold" /></div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div><label className="block text-xs font-black uppercase mb-1 italic">개체값 (IV)</label><input type="number" value={iv} onChange={(e) => setIv(Number(e.target.value))} className="w-full bg-gray-100 p-3 rounded-xl font-bold" /></div>
            </div>
            <div><label className="block text-xs font-black uppercase mb-1 italic">노력치 (EV)</label><input type="number" value={ev} step={4} max={252} onChange={(e) => setEv(Number(e.target.value))} className="w-full bg-gray-100 p-3 rounded-xl font-bold" /></div>
          </div>
