...
type View = 'home' | 'mbti' | 'speed' | 'damage' | 'matchup' | 'coverage' | 'tiers' | 'priority' | 'counter';
    ...
    return (
      <Suspense fallback={
        <div className="flex flex-col items-center justify-center min-h-[400px]">
          <Loader2 size={48} className="animate-spin text-poke-yellow mb-4" />
          <p className="font-black text-white uppercase italic animate-pulse">Loading Tool...</p>
        </div>
      }>
        {(() => {
          switch (view) {
              ...
              return (
                <div className="w-full max-w-7xl animate-in fade-in duration-500 pb-20 md:pb-0">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-8">
                    ...
                    <MenuButton onClick={() => handleNav('priority')} icon="🚀" title="우선도 가이드" sub="기술 선제권" borderColor="border-cyan-500" bgColor="bg-cyan-500" label="Move Priority" />
                    <MenuButton onClick={() => handleNav('counter')} icon="🎯" title="카운터 분석" sub="공략법 찾기" borderColor="border-pink-500" bgColor="bg-pink-500" label="Counter Analysis" />
                  </div>
                </div>
              );
          }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-lg border-t-4 border-poke-dark flex justify-around items-center p-3 z-50 md:hidden shadow-[0_-10px_20px_rgba(0,0,0,0.2)]">
        ...
        <NavButton icon={<Search />} label="계산기" active={view === 'speed' || view === 'damage'} onClick={() => handleNav('speed')} />
        <NavButton icon={<Star />} label="분석" active={view === 'coverage' || view === 'counter'} onClick={() => handleNav('coverage')} />
        <NavButton icon={<User />} label="가이드" active={view === 'tiers' || view === 'priority'} onClick={() => handleNav('tiers')} />
      </nav>
...
const MenuButton = ({ onClick, icon, title, sub, borderColor, bgColor, label }: MenuButtonProps) => (
  ...
  >
    <div className="flex flex-col items-center gap-2 md:gap-3 text-poke-dark">
      ...
      <h2 className="text-[12px] md:text-xl font-black uppercase tracking-tighter leading-none">{title}</h2>
      <p className="font-bold text-gray-400 text-[8px] md:text-xs text-center leading-tight">{sub}</p>
      <div className={`mt-1 md:mt-2 px-3 md:px-4 py-1 bg-poke-dark text-white font-black rounded-full text-[7px] md:text-[10px] uppercase italic`}>{label}</div>
    </div>
