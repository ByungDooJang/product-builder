...
type View = 'home' | 'mbti' | 'speed' | 'damage' | 'matchup' | 'coverage' | 'tiers' | 'priority';
        ...
        return (
          <div className="w-full max-w-7xl animate-in fade-in duration-500 pb-20 md:pb-0">
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
              ...
              <div className="hidden lg:flex bg-white/5 border-2 border-dashed border-white/10 rounded-3xl items-center justify-center text-gray-500 font-black uppercase italic text-xs">More Coming Soon...</div>
            </div>
          </div>
        );
    }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <nav className="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-lg border-t-4 border-poke-dark flex justify-around items-center p-3 z-50 md:hidden shadow-[0_-10px_20px_rgba(0,0,0,0.2)]">
        ...
        <NavButton icon={<Star />} label="분석" active={view === 'coverage' || view === 'tiers'} onClick={() => handleNav('coverage')} />
        <NavButton icon={<User />} label="가이드" active={view === 'mbti' || view === 'priority'} onClick={() => handleNav('mbti')} />
      </nav>
