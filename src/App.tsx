...
import { Info, TrendingUp, Home, Search, Star, User, Loader2, Trophy, Settings, X, Zap, Heart, List, Grid3X3, Grid } from 'lucide-react';
...
type View = 'home' | 'mbti' | 'speed' | 'damage' | 'matchup' | 'coverage' | 'tiers' | 'priority' | 'counter' | 'settings' | 'log' | 'partnership' | 'heatmap' | 'matrix';
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-12">
                    <div className="lg:col-span-8 space-y-6">
                       <div className="bg-white/5 backdrop-blur-md border-2 border-white/10 rounded-3xl p-6 flex items-start gap-4 shadow-xl">
                          ...
                          <div><h3 className="text-sm font-black uppercase text-poke-yellow mb-1 italic">Master's Secret Tip</h3><p className="text-lg font-bold text-gray-200 leading-snug">"{tips[tipIndex]}"</p></div>
                       </div>
  ...
  return (
    <div className="flex flex-col min-h-screen transition-all duration-500" style={{ backgroundColor: themes[themeId].bg }}>
      ...
      <footer className="bg-poke-dark p-6 text-center border-t-2 border-white/10 pb-24 md:pb-6">
        <p className="text-gray-500 font-bold text-[10px] uppercase tracking-widest">Pokémon Champions v10.0 Legendary Pro</p>
      </footer>
    </div>
  );
};
